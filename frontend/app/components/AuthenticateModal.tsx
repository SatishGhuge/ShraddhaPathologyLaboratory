...
const AuthenticateModal = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Check if click is outside all dropdowns and textareas
      const isClickInsideAny = Object.keys(dropdownRefs.current).some(paramId => {
        const dropdownElement = dropdownRefs.current[paramId];
        const textareaElement = inputRefs.current[paramId];
        
        if (!dropdownElement || !textareaElement) return false;
        
        return dropdownElement.contains(target) || textareaElement.contains(target);
      });
      
      // If clicked outside, close all dropdowns
      if (!isClickInsideAny && Object.values(openDropdowns).some(Boolean)) {
        setOpenDropdowns({});
      }
    };
                                  ...
                                  <div className="w-full relative">
                                    <textarea
                                      ...
                                      onChange={(e) => {
                                        const newResults = { ...results };
                                        if (!newResults[param.id]) newResults[param.id] = {};
                                        newResults[param.id].textValue = e.target.value.replace(/,\s*/g, '|');
                                        setResults(newResults);
                                      }}
                                      onFocus={() => setOpenDropdowns({ ...openDropdowns, [param.id]: true })}
                                      onBlur={() => setTimeout(() => setOpenDropdowns({ ...openDropdowns, [param.id]: false }), 200)}
                                      placeholder=""
                                        ...
                                        {getAllOptionsFromParameter(param).map((option: string) => (
                                          <div
                                            ...
                                            onMouseDown={() => {
                                              ...
                                              const newValue = existing ? `${existing}|${option}` : option;
