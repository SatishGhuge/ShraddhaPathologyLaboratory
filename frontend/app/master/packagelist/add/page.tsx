...
const AddPackage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('   - testIds:', testList.map(t => t.id));
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border border-cyan-400 rounded-md shadow-lg max-h-60 overflow-y-auto">
                      {searchResults.map((test) => (
                        <div
                          ...
                          className="px-3 py-2 text-xs sm:text-sm hover:bg-gray-50 cursor-pointer border-b last:border-b-0 flex justify-between items-center"
                        >
                          <span>{test.name}</span>
