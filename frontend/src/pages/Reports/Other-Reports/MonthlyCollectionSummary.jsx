...
import { getMonthlyCollectionSummary } from "../../../api/admin.js";
import { getCollectionCenters } from "../../../api/patient.js";
...
const toGB = (iso) => { if(!iso) return "-"; return new Date(iso).toLocaleDateString("en-GB"); };
...
export default function MonthlyCollectionSummary() {
  ...
  const toggleCol  = (id) => setSelectedColumns(p => p.includes(id) ? p.filter(x=>x!==id) : [...p, id]);
  ...
  const has        = (id) => selectedColumns.includes(id);
  const total      = (key) => data.reduce((s,r) => s + (r[key]||0), 0);
