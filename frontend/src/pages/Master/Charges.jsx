...
export default function AddLabCharges() {
  ...
  const fetchTestsAndCharges = async () => {
    try {
      ...
      // Fetch all tests
      const testsResponse = await fetch(`${import.meta.env.VITE_API_URL}/master/tests`);
      ...
      if (testsResult.success) {
        ...
        const chargesPromises = testsResult.data.map(async (test) => {
          try {
            const chargesResponse = await fetch(`${import.meta.env.VITE_API_URL}/master/tests/${test.id}/charges`);
  ...
  useEffect(() => {
    ...
    const result = tests.map(test => {
      const testCharges = charges.find(c => c.testId === test.id)?.charges || [];
      const defaultCharge = testCharges.find(c => !c.franchiseId && !c.corporateId && !c.collectionCenterId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/master/test-charges/bulk`, {
        method: 'POST',
        headers: {
      ...
      <div className="p-6 bg-cyan-100 min-h-screen">
        ...
        <div className="bg-white rounded shadow-md border border-gray-200">
          ...
          <div className="border-b border-gray-300 p-4">
            ...
            <button 
              onClick={() => navigate("/master/corporate-wise-charges")}
