...
import { getCollectionCenterById, createCollectionCenter, updateCollectionCenter } from "../../api/master.js";
...
const AddCenter = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await updateCollectionCenter(id, formData);
        ...
        setToast({ type: "success", message: "Center added successfully!", credentials: res.credentials || null });
      }
    } catch (err) {
      setToast({ type: "error", message: err.message || "Failed to save center" });
    } finally {
      setSaving(false);
    }
  };
  ...
  const closeToast = () => {
    ...
    setToast(null);
    if (wasSuccess) navigate("/master/centerlist");
  };
      ...
      <div className="p-6 bg-cyan-50 min-h-screen flex justify-center">
        <div className="bg-white rounded-lg shadow py-3 px-4 w-full max-w-2xl h-fit">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <button type="button" onClick={() => navigate("/master/centerlist")} className="p-1.5 rounded-full bg-cyan-100 hover:bg-cyan-200">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            ...
            {!isViewMode && (
              <div className="md:col-span-2 flex gap-3 mt-2">
                ...
                <button type="button" onClick={() => navigate("/master/centerlist")}
