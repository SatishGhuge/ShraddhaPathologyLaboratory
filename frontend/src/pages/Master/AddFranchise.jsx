...
import { updateFranchise, getFranchiseById, createFranchiseWithCredentials } from "../../api/master.js";

/* ── small toast popup ── */
const Toast = ({ type, message, credentials, onClose }) => (
...
const AddFranchise = () => {
  ...
  const [formData, setFormData] = useState({
    name: "", franchiseType: "", address: "", code: "",
    location: "", mobile: "", email: "", date: "", active: "Yes",
  });

  const [toast, setToast] = useState(null); // { type, message, credentials }
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (id && (isViewMode || isEditMode)) {
      getFranchiseById(id).then(franchise => {
        if (franchise) {
          setFormData({
            name: franchise.name || "",
            ...
            date: franchise.date ? new Date(franchise.date).toISOString().split("T")[0] : "",
            active: franchise.isActive ? "Yes" : "No",
          });
        }
      }).catch(console.error);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        setToast({ type: "success", message: "Franchise updated successfully! Update notification sent to email." });
      } else {
        const res = await createFranchiseWithCredentials(payload);
        setToast({
          type: "success",
      ...
      setToast({ type: "error", message: err.message || "Failed to save franchise" });
    } finally {
      setSaving(false);
    }
  };

  const getTitle = () => isViewMode ? "VIEW FRANCHISE" : isEditMode ? "EDIT FRANCHISE" : "ADD FRANCHISE";
