...
import { getCorporates, deleteCorporate, updateCorporate } from "../../api/master.js";
...
const CorporateList = () => {
  ...
  const fetchCorporates = async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to load corporates");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <div className="p-6 bg-cyan-50 min-h-screen">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4 bg-white p-3 rounded shadow-md">
          ...
          <button onClick={() => navigate("/master/corporate/add")}
                ...
                <tr key={c.id} className="hover:bg-gray-50 border-b border-gray-200">
                  ...
                  <td className="border border-gray-300 px-3 py-1">
                    <div className="flex gap-1 flex-wrap">
                      <button onClick={() => navigate(`/master/corporate/view/${c.id}`)}
                        className="bg-cyan-600 text-white px-2 py-1 rounded text-xs hover:bg-cyan-700">View</button>
                      <button onClick={() => navigate(`/master/corporate/edit/${c.id}`)}
