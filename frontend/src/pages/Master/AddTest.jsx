...
import { getTestById, createTest, updateTest, getDepartments, getUnits, getTests } from "../../api/master.js";
...
const AddTest = () => {
  ...
  const isEditMode = location.pathname.toLowerCase().includes('/edit/') || location.pathname.toLowerCase().includes('/edit');
  const isViewMode = location.pathname.toLowerCase().includes('/view/') || location.pathname.toLowerCase().includes('/view');
  ...
  const handleParamNameSearch = (catIdx, paramIdx, value) => {
  ...
  const applyParamSuggestion = (catIdx, paramIdx, suggestion) => {
    const key = `${catIdx}-${paramIdx}`;
  ...
  const getFormulaKey = (catIdx, paramIdx) => `${catIdx}-${paramIdx}`;

  const updateDraft = (catIdx, paramIdx, value) => {
  ...
  const generateCategoryKey = (index) => {
                        ...
                        return {
                          ...
                          normalRanges: param.normalRanges && param.normalRanges.length > 0
                            ? param.normalRanges.map(range => ({
                                gender: range.gender,
                                ll: (range.ll || range.lowValue)?.toString() || "",
                                ul: (range.ul || range.highValue)?.toString() || "",
  ...
  const addCategory = () => {
    setCategories([...categories, {
      categoryId: crypto.randomUUID(), // ✅ Unique ID for each new category
      name: "",
      categoryType: "PARAMETER",
      isCategory: false,
      sortOrder: "",
  ...
  const addParameter = (categoryIndex) => {
    const updatedCategories = [...categories];
  ...
  const deleteParameter = (categoryIndex, parameterIndex) => {
    const updatedCategories = [...categories];
    ...
    if (updatedCategories[categoryIndex].parameters.length === 0) {
      updatedCategories[categoryIndex].parameters.push({
        ...
        ageRanges: [
          ...
          { label: "More Than For Male", value: "", ll: "", ul: "", default: "", timeUnit: "Day(s)", isActive: false },
          { label: "Less Than For Female", value: "", ll: "", ul: "", default: "", timeUnit: "Day(s)", isActive: false },
          { label: "More Than For Female", value: "", ll: "", default: "", timeUnit: "Day(s)", isActive: false },
          { label: "Between Male", from: "", to: "", ll: "", ul: "", default: "", timeUnit: "Day(s)", isActive: false }
        ],
        rangeValues: [
          { label: "Less Than", min: "", max: "", interpretation: "", isActive: false },
  ...
  const handleAgeRangeChange = (categoryIndex, parameterIndex, rangeIndex, field, value) => {
    ...
    if (field === 'isActive' && value === true) {
      const ageRange = updatedCategories[categoryIndex].parameters[parameterIndex].ageRanges[rangeIndex];
  ...
  const addAgeRange = (categoryIndex, parameterIndex) => {
    const updatedCategories = [...categories];
  ...
  const removeAgeRange = (categoryIndex, parameterIndex, rangeIndex) => {
    const updatedCategories = [...categories];
  ...
  const addRangeValue = (categoryIndex, parameterIndex) => {
    const updatedCategories = [...categories];
  ...
  const removeRangeValue = (categoryIndex, parameterIndex, rangeIndex) => {
    const updatedCategories = [...categories];
    ...
    try {
      ...
      const categoryData = categories.filter(category => 
        ...
        parameters: category.parameters ? category.parameters.filter(param => 
          ...
          ageRanges: (() => {
            ...
            return param.ageRanges ? param.ageRanges.map(ageRange => ({
              label: ageRange.label,
              gender: ageRange.gender || null,
              enabled: ageRange.isActive,
              value: ageRange.value ? parseFloat(ageRange.value) : null,
              from: ageRange.from ? parseFloat(ageRange.from) : null,
              to: ageRange.to ? parseFloat(ageRange.to) : null,
      ...
      if (isAddMode) {
        ...
        if (goToCharges && testId) {
          navigate(`/master/test-charges/${testId}`);
        } else {
          navigate("/master/testlist");
        }
      } else if (isEditMode) {
        console.log("✏️ Updating existing test with ID:", id);
        const response = await updateTest(id, completeTestData);
        ...
        if (goToCharges) {
          navigate(`/master/test-charges/${id}`);
        } else {
          navigate("/master/testlist");
        }
      }
                    ...
                    {!isViewMode && (
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            const interpretationContainer = document.querySelector('#interpretation-editor');
                            const expandBtn = document.querySelector('#interpretation-expand-btn');
                            const minimizeBtn = document.querySelector('#interpretation-minimize-btn');
                      ...
                      {editorLoaded ? (
                        <CKEditor
                          ...
                          onChange={(event, editor) => {
                            ...
                            handleChange({ target: { name: 'interpretation', value: data } });
                          }}
                          onReady={(editor) => {
                ...
                <div className="flex-1 space-y-3">
                  ...
                  {category.parameters && category.parameters.map((parameter, paramIndex) => (
                    <div key={paramIndex} className="border border-gray-200 rounded p-3 bg-white mb-3">
                          ...
                          {!isViewMode && (
                            <div className="flex gap-1">
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  const input = document.getElementById(`param-name-${categoryIndex}-${paramIndex}`);
                      ...
                      {!!(parameter.hasFormula && parameter._editingFormula) && (
                      ...
                      {(!parameter.hasFormula || !isEditMode || !parameter._editingFormula) && (
                              ...
                              <div className="flex justify-between items-center bg-gray-50 px-3 py-2 border-b border-gray-300">
                                ...
                                <div className="flex gap-2">
                                  <button
                                    type="button"
                                    onClick={() => {
                                      const editorContainer = document.querySelector(`#editor-${categoryIndex}-${paramIndex}`);
                                      const expandBtn = document.querySelector(`#expand-btn-${categoryIndex}-${paramIndex}`);
                                      const minimizeBtn = document.querySelector(`#minimize-btn-${categoryIndex}-${paramIndex}`);
                                          ...
                                          {param.normalRanges?.map((range, idx) => (
                                            range.isActive && (
                                              <div key={idx} className="ml-2">
                                                {range.lowValue !== undefined && range.lowValue !== null ? range.lowValue : (range.ll || "-")} - {range.highValue !== undefined && range.highValue !== null ? range.highValue : (range.ul || "-")}
                                            ...
                                            param.ageRanges.map((ageRange, idx) => {
                                              ...
                                              const isInRange = (() => {
                                                if (ageRange.label && ageRange.label.includes("Between")) {
                                                  const from = parseInt(ageRange.from);
                                                  const to = parseInt(ageRange.to);
                                                ...
                                                if (ageRange.label && (ageRange.label.includes("Less Than") || ageRange.label.includes("More Than"))) {
                                                  const value = parseInt(ageRange.value);
