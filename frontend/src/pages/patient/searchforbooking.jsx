...
import { getAllPatients, updatePayment, updatePatient } from "../../api/patient.js";
import { getDoctors, getTests, getPackages, getSpecimenTypes } from "../../api/master.js";
import html2pdf from "html2pdf.js";
import { jsPDF } from "jspdf";
import LetterHead from "../../assets/LetterHead.jpeg";
import { generateBillPDF, printBill } from "../../utils/billPdfGenerator.js";
...
export default function BookingPage() {
  ...
  useEffect(() => {
    const incomingUid = location?.state?.searchQuery;
  ...
  }, [allBookings, location?.state?.searchQuery]);
  ...
  const validateReferral = () => {
    ...
    if (referralData.compliment && (isNaN(referralData.compliment) || parseFloat(referralData.compliment) < 0 || parseFloat(referralData.compliment) > 100)) {
  ...
  const handleSaveCharge = (testName) => {
  ...
  const handleRebooking = (booking) => {
    navigate('/patient/registration', {
      state:{ patientData:booking.patientData, isRebooking:true, previousBookingId:booking.visitId || booking.bookingId }
    });
  };
            ...
            <div className="p-6 space-y-4">
              ...
              {[
                ...
                <div key={i} className={style.formGrid}>
                  ...
                  {f[2]==="text"
                    ? <input type="text" name={f[1]} value={formData[f[1]]} onChange={handleInputChange} className="col-span-9 border border-cyan-400 rounded px-3 py-2 bg-cyan-50"/>
                    : <select name={f[1]} value={formData[f[1]]} onChange={handleInputChange} className="col-span-9 border border-cyan-400 rounded px-3 py-2 bg-cyan-50">
                        {(f[3]||[formData[f[1]]]).map((opt,j)=><option key={j}>{opt}</option>)}
                      </select>}
                </div>
              ))}
              <div className={style.formGrid}>
