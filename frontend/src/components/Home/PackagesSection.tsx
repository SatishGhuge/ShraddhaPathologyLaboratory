  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <div className="bg-gradient-to-br from-[oklch(45%_0.085_224.283)] to-[oklch(40%_0.075_224.283)] p-6 relative">
            {pkg.isPopular && (
              <span className="inline-flex items-center gap-1 bg-[oklch(60%_0.15_45)] text-white text-[10px] font-bold px-3 py-1 rounded-full mb-3">
                <Star size={9} fill="white" /> MOST POPULAR
              </span>
            )}
            <h2 className="text-xl font-bold text-white mb-2">{pkg.name}</h2>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold text-[oklch(60%_0.15_45)]">₹{pkg.price}</span>
              {pkg.originalPrice && (
          ...
          <div className="p-6">
            <div className="flex items-center gap-2 mb-4">
              <FlaskConical size={16} className="text-[oklch(60%_0.15_45)]" />
            ...
            <ul className="space-y-2 mb-6 max-h-52 overflow-y-auto pr-1">
              {tests.map((t) => (
                <li key={t} className="flex items-center gap-2.5 text-sm text-[#475569]">
                  <CheckCircle size={13} className="text-[oklch(60%_0.15_45)] flex-shrink-0" />
                  {t}
                </li>
              ))}
            </ul>
            <button className="w-full bg-[oklch(60%_0.15_45)] hover:bg-[oklch(55%_0.14_45)] text-white font-bold py-3.5 rounded-2xl transition-colors text-sm">
              Book Now →
            </button>
          </div>
        </motion.div>
    ...
    >
      ...
      {pkg.isPopular && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-20 whitespace-nowrap">
          <span className="inline-flex items-center gap-1.5 bg-[oklch(60%_0.15_45)] text-white text-[11px] font-bold px-4 py-1.5 rounded-full shadow-xl">
      ...
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 ${
        pkg.isPopular ? "bg-white/15" : "bg-[oklch(45%_0.085_224.283)]/08"
      }`}
        style={{ background: pkg.isPopular ? "rgba(255,255,255,0.15)" : "rgba(47,86,120,0.08)" }}
      >
        <FlaskConical size={26} className={pkg.isPopular ? "text-white" : "text-[oklch(45%_0.085_224.283)]"} />
      </div>
