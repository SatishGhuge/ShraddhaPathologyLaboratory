...
// ✅ Settlement Save Endpoint - From Collection Report Settlement Form
export const saveSettlement = async (req, res) => {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      // Step 1: Create BillingSession for SETTLEMENT
      const lastSession = await tx.billingSession.findFirst({
        where: { visitId },
        orderBy: { sequence: 'desc' }
      });
      ...
      const settlementSession = await tx.billingSession.create({
        data: {
          ...
          sequence: nextSequence,
          remarks: `Settlement: Org Discount ₹${finalOrgDiscount}, TDS ₹${Math.round(tdsAmount * 100) / 100}, Other Discount ₹${Math.round(finalOtherDiscount * 100) / 100}`
        }
      });

      console.log(`✅ [TX] BillingSession created: SETTLEMENT, sequence=${nextSequence}`);

      // Step 2: Create BillDiscount for Organization Discount
      if (finalOrgDiscount > 0) {
        await tx.billDiscount.create({
          data: {
            ...
            discountType: 'FLAT',
            discountValue: new Decimal(finalOrgDiscount.toString()),
            discountAmount: new Decimal(finalOrgDiscount.toString()),
            appliedOnAmount: new Decimal(grandTotal.toString()),
            remarks: `Organization discount from ${organization.name}`
          }
        });
        console.log(`✅ [TX] Organization Discount created: ₹${finalOrgDiscount}`);
      }

      // Step 3: Create BillDiscount for TDS
      if (tdsAmount > 0) {
        await tx.billDiscount.create({
          data: {
            visitId,
            ...
            discountValue: new Decimal((tdsPercent || 10).toString()),
            discountAmount: new Decimal(Math.round(tdsAmount * 100) / 100),
            appliedOnAmount: new Decimal(afterOrgDiscount.toString()),
            remarks: `TDS ${tdsPercent}% deducted`
          }
        });
        console.log(`✅ [TX] TDS created: ₹${Math.round(tdsAmount * 100) / 100}`);
      }

      // Step 4: Create BillDiscount for Other Discount
      if (finalOtherDiscount > 0) {
        await tx.billDiscount.create({
          data: {
            visitId,
            billingSessionId: settlementSession.id,
            discountType: otherDiscountPercent ? 'PERCENTAGE' : 'FLAT',
            discountValue: new Decimal(otherDiscountPercent ? otherDiscountPercent.toString() : finalOtherDiscount.toString()),
            discountAmount: new Decimal(Math.round(finalOtherDiscount * 100) / 100),
            appliedOnAmount: new Decimal(afterOrgDiscount.toString()),
            remarks: `Other discount${otherDiscountPercent ? ` ${otherDiscountPercent}%` : ''}`
          }
        });
        console.log(`✅ [TX] Other Discount created: ₹${Math.round(finalOtherDiscount * 100) / 100}`);
      }

      // Step 5: Create Payment record if amount paid > 0
      if (paymentAmount > 0) {
        await tx.payment.create({
          data: {
            visitId,
            billingSessionId: settlementSession.id,
            amount: new Decimal(paymentAmount.toString()),
            paymentMode: 'CASH', // Default to cash for settlement
            transactionStatus: 'SUCCESS',
            remarks: remark || 'Settlement payment',
            paymentDate: new Date()
          }
        });
        console.log(`✅ [TX] Payment created: ₹${paymentAmount}`);
      ...
      // Step 6: Update VisitBill with final amounts and PAID status
      const totalDiscountAmount = visitBill.totalDiscount.toNumber() + finalOrgDiscount + tdsAmount + finalOtherDiscount;
      const newTotalPaid = visitBill.totalPaid.toNumber() + paymentAmount;
      const finalBalance = Math.max(0, grandTotal - totalDiscountAmount - newTotalPaid);

      updatedBill = await tx.visitBill.update({
        where: { visitId },
        data: {
          grossAmount: new Decimal(grandTotal.toString()),
          totalDiscount: new Decimal(totalDiscountAmount.toString()),
          totalDiscountPercent: grandTotal > 0 ? Math.round((totalDiscountAmount / grandTotal) * 100) : 0,
          totalPaid: new Decimal(newTotalPaid.toString()),
          balanceAmount: new Decimal(finalBalance.toString()),
          status: billStatus
        }
      ...
      console.log(`✅ [TX] VisitBill updated: status=${billStatus}, balance=₹${finalBalance}`);
    });
...
// ✅ BULK PATIENT SETTLEMENT - Settle ALL visits for a patient under an organization
      ...
      for (const visitBill of visitBills) {
        ...
        const settlementSession = await tx.billingSession.create({
          data: {
            ...
            sessionType: 'SETTLEMENT',
            sequence: nextSequence,
            remarks: `Bulk settlement: Org Discount ₹${finalOrgDiscount}, TDS ₹${Math.round(tdsAmount * 100) / 100}, Other Discount ₹${Math.round(finalOtherDiscount * 100) / 100}`
          }
        });

        console.log(`✅ [TX] BillingSession created for visit ${visitId}: sequence=${nextSequence}`);

        // Step 2: Create BillDiscount for Organization Discount (proportional to this visit's amount)
        const proportionalOrgDiscount = (visitBill.grossAmount.toNumber() / totalGrossAmount) * finalOrgDiscount;
        
        if (proportionalOrgDiscount > 0) {
          await tx.billDiscount.create({
            data: {
              visitId,
              billingSessionId: settlementSession.id,
              discountType: 'FLAT',
              discountValue: new Decimal(proportionalOrgDiscount.toString()),
              discountAmount: new Decimal(proportionalOrgDiscount.toString()),
              appliedOnAmount: new Decimal(visitBill.grossAmount.toString()),
              remarks: `Organization bulk discount from ${organization.name}`
            }
          });
          console.log(`✅ [TX] Organization Discount created for visit ${visitId}: ₹${proportionalOrgDiscount}`);
        }

        // Step 3: Create BillDiscount for TDS (proportional to this visit)
        const proportionalTds = (visitBill.grossAmount.toNumber() / totalGrossAmount) * tdsAmount;
        ...
        if (proportionalTds > 0) {
          await tx.billDiscount.create({
            data: {
              ...
              discountType: 'PERCENTAGE',
              discountValue: new Decimal((tdsPercent || 10).toString()),
              discountAmount: new Decimal(Math.round(proportionalTds * 100) / 100),
              appliedOnAmount: new Decimal(((visitBill.grossAmount.toNumber() - proportionalOrgDiscount) / totalGrossAmount * afterOrgDiscount).toString()),
              remarks: `TDS ${tdsPercent}% deducted (bulk settlement)`
            }
          });
          console.log(`✅ [TX] TDS created for visit ${visitId}: ₹${Math.round(proportionalTds * 100) / 100}`);
        }

        // Step 4: Create BillDiscount for Other Discount (proportional)
        const proportionalOtherDiscount = (visitBill.grossAmount.toNumber() / totalGrossAmount) * finalOtherDiscount;
        
        if (proportionalOtherDiscount > 0) {
          await tx.billDiscount.create({
            data: {
              visitId,
              billingSessionId: settlementSession.id,
              discountType: otherDiscountPercent ? 'PERCENTAGE' : 'FLAT',
              discountValue: new Decimal(otherDiscountPercent ? otherDiscountPercent.toString() : proportionalOtherDiscount.toString()),
              discountAmount: new Decimal(Math.round(proportionalOtherDiscount * 100) / 100),
              appliedOnAmount: new Decimal(afterOrgDiscount.toString()),
              remarks: `Other discount${otherDiscountPercent ? ` ${otherDiscountPercent}%` : ''} (bulk settlement)`
            }
          });
          console.log(`✅ [TX] Other Discount created for visit ${visitId}: ₹${Math.round(proportionalOtherDiscount * 100) / 100}`);
        }

        // Step 5: Create Payment record (proportional payment allocation)
        const proportionalPayment = (visitBill.grossAmount.toNumber() / totalGrossAmount) * paymentAmount;
        ...
        if (proportionalPayment > 0) {
          await tx.payment.create({
            data: {
              visitId,
              billingSessionId: settlementSession.id,
              amount: new Decimal(proportionalPayment.toString()),
              paymentMode: 'CASH',
              transactionStatus: 'SUCCESS',
              remarks: remark || 'Bulk settlement payment',
              paymentDate: new Date()
            }
          });
          console.log(`✅ [TX] Payment created for visit ${visitId}: ₹${proportionalPayment}`);
        }

        // Step 6: Update VisitBill with final amounts and PAID status
        const totalDiscountForVisit = visitBill.totalDiscount.toNumber() + proportionalOrgDiscount + proportionalTds + proportionalOtherDiscount;
        const newTotalPaid = visitBill.totalPaid.toNumber() + proportionalPayment;
        const grossAmount = visitBill.grossAmount.toNumber();
        const finalBalance = Math.max(0, grossAmount - totalDiscountForVisit - newTotalPaid);

        const updatedBill = await tx.visitBill.update({
          where: { visitId },
          data: {
            grossAmount: new Decimal(grossAmount.toString()),
            totalDiscount: new Decimal(totalDiscountForVisit.toString()),
            totalDiscountPercent: grossAmount > 0 ? Math.round((totalDiscountForVisit / grossAmount) * 100) : 0,
            totalPaid: new Decimal(newTotalPaid.toString()),
            balanceAmount: new Decimal(finalBalance.toString()),
            status: proportionalPayment >= (grossAmount - totalDiscountForVisit) ? 'PAID' : proportionalPayment > 0 ? 'PARTIAL' : 'PENDING'
          }
        });

        updatedBills.push(updatedBill);
        console.log(`✅ [TX] VisitBill updated for ${visitId}: balance=₹${finalBalance}, status=${updatedBill.status}`);
      }
    });

    console.log(`✅ Bulk settlement saved successfully for ${visitIds.length} visits`);

    res.json({
      success: true,
      message: `Bulk settlement saved successfully for ${visitIds.length} visit(s)`,
      data: {
        patientId,
        orgId,
        visitIds,
        billStatus,
        visitBills: updatedBills.map(vb => ({
          visitId: vb.visitId,
          grossAmount: vb.grossAmount.toNumber ? vb.grossAmount.toNumber() : parseFloat(vb.grossAmount),
          totalDiscount: vb.totalDiscount.toNumber ? vb.totalDiscount.toNumber() : parseFloat(vb.totalDiscount),
          totalPaid: vb.totalPaid.toNumber ? vb.totalPaid.toNumber() : parseFloat(vb.totalPaid),
          balanceAmount: vb.balanceAmount.toNumber ? vb.balanceAmount.toNumber() : parseFloat(vb.balanceAmount),
          status: vb.status
        }))
      }
    });
...
// ✅ ORGANIZATION-WIDE SETTLEMENT - Settle ALL visits under an organization
export const saveOrgSettlement = async (req, res) => {
  try {
    ...
    // Validate inputs
    if (!orgId || visitIds.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'orgId and visitIds are required'
      });
    }

    console.log('🏢 Organization-Wide Settlement Request:', {
      orgId,
      visitIds,
      visitCount: visitIds.length,
      orgDiscount,
    ...
    // ✅ Calculate Combined Settlement Amounts for entire org
    // Sum all gross amounts from all visits
    const totalGrossAmount = visitBills.reduce((sum, vb) => sum + vb.grossAmount.toNumber(), 0);
    const finalOrgDiscount = Math.min(orgDiscount || 0, totalGrossAmount);
    const afterOrgDiscount = totalGrossAmount - finalOrgDiscount;
    ...
    console.log('💰 Organization Settlement Calculation:', {
      totalGrossAmount,
      finalOrgDiscount,
      afterOrgDiscount,
      tdsAmount: Math.round(tdsAmount * 100) / 100,
      ...
      for (const visitBill of visitBills) {
        ...
        const settlementSession = await tx.billingSession.create({
          data: {
            ...
            sessionType: 'SETTLEMENT',
            sequence: nextSequence,
            remarks: `Organization settlement: Org Discount ₹${finalOrgDiscount}, TDS ₹${Math.round(tdsAmount * 100) / 100}, Other Discount ₹${Math.round(finalOtherDiscount * 100) / 100}`
          }
        });
        ...
        if (proportionalOrgDiscount > 0) {
          await tx.billDiscount.create({
            data: {
              ...
              discountAmount: new Decimal(proportionalOrgDiscount.toString()),
              appliedOnAmount: new Decimal(visitBill.grossAmount.toString()),
              remarks: `Organization settlement discount from ${organization.name}`
            }
          });
          console.log(`✅ [TX] Organization Discount created for visit ${visitId}: ₹${proportionalOrgDiscount}`);
        }
        ...
        if (proportionalTds > 0) {
          await tx.billDiscount.create({
            data: {
              ...
              discountAmount: new Decimal(Math.round(proportionalTds * 100) / 100),
              appliedOnAmount: new Decimal(((visitBill.grossAmount.toNumber() - proportionalOrgDiscount) / totalGrossAmount * afterOrgDiscount).toString()),
              remarks: `TDS ${tdsPercent}% deducted (org settlement)`
            }
          });
          console.log(`✅ [TX] TDS created for visit ${visitId}: ₹${Math.round(proportionalTds * 100) / 100}`);
        }
        ...
        if (proportionalOtherDiscount > 0) {
          await tx.billDiscount.create({
            data: {
              ...
              discountAmount: new Decimal(Math.round(proportionalOtherDiscount * 100) / 100),
              appliedOnAmount: new Decimal(afterOrgDiscount.toString()),
              remarks: `Other discount${otherDiscountPercent ? ` ${otherDiscountPercent}%` : ''} (org settlement)`
            }
          });
          console.log(`✅ [TX] Other Discount created for visit ${visitId}: ₹${Math.round(proportionalOtherDiscount * 100) / 100}`);
        }
        ...
        if (proportionalPayment > 0) {
          await tx.payment.create({
            data: {
              ...
              paymentMode: 'CASH',
              transactionStatus: 'SUCCESS',
              remarks: remark || 'Organization settlement payment',
              paymentDate: new Date()
            }
          });
          console.log(`✅ [TX] Payment created for visit ${visitId}: ₹${proportionalPayment}`);
    ...
    console.log(`✅ Organization settlement saved successfully for ${visitIds.length} visits`);

    res.json({
      success: true,
      message: `Organization settlement complete! Settled ${visitIds.length} visit(s)`,
      data: {
        orgId,
        visitIds,
        billStatus,
    ...
    console.error('Organization settlement save error:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to save organization settlement',
      error: error.message
    });
  }
};
